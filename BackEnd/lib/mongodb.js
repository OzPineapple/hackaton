...
driver.getUsers = () => { return driver.admin_getAll(); }
driver.newUsers	 = ( reqBody ) => { return driver.admin_get(reqBody); }
driver.upUsers	 = ( reqBody ) => { return driver.admin_update(reqBody); }
driver.getUsers	 = ( id ) => { return driver.admin_getByID(id); }
driver.rmUsers = ( id ) => { throw new CustomError("NotCodedYet", null)};
...
driver.usr_set = async ({arg_name, arg_password, arg_mail, arg_type, arg_rfc, arg_private_key_58, arg_clabe}) => {
	var size = 0;

	size = await collUsuario.count();
	size++;

	var newUsuario = {id_text: size, name: arg_name, password: arg_password, mail: arg_mail, type: arg_type, rfc: arg_rfc, private_key_58: arg_private_key_58, clabe: arg_clabe};
	
	await collUsuario.insertOne(newUsuario);
	console.log("Usuario Registrado");
}

driver.usr_getByPrivateK = async (arg_private_key_58) => {
	const query = { private_key_58: arg_private_key_58 };
	const options =  {projection: {_id: 0, password: 0, private_key_58: 0}};
	...
	if( count == 0 )
		throw new CustomStatusError( "UserNotFound", 404, 
			"La private key " + arg_private_key_58 + " no se encuentra en la base de datos " + process.env.npm_package_config_dbname
		);
	else if( count > 1 )
		throw new CustomStatusError( "Duplicated Record", 409,
			"Demasiados usuarios duplicados, deberían ser únicos para la busqueda " + arg_private_key_58
		);
	else if (count == 1)
		return users.next();
	else
...
driver.usr_getByMail = async (arg_mail) => {
	const query = { mail: arg_mail };
	const options =  {projection: {_id: 0, password: 0, private_key_58: 0}};
	...
	if( count == 0 )
		throw new CustomStatusError( "UserNotFound", 404, 
			"El correo " + arg_mail + " no se encuentra en la base de datos " + process.env.npm_package_config_dbname
		);
	else if( count > 1 )
		throw new CustomStatusError( "Duplicated Record", 409,
			"Demasiados usuarios duplicados, deberían ser únicos para la busqueda " + arg_mail
		);
	else if (count == 1)
		return users.next();
	else
...
driver.usr_getByMailL = async (arg_mail) => {
	const query = { mail: arg_mail };
	const options =  {projection: {_id: 0, id_text: 1, password: 1}};
...
driver.usr_getByID = async (arg_id_text) => {
	const query = { id_text: arg_id_text };
	...
	if( count == 0 )
		throw new CustomStatusError( "UserNotFound", 404, 
			"El usuario " + arg_id_text + " no se encuentra en la base de datos " + process.env.npm_package_config_dbname
		);
	else if( count > 1 )
		throw new CustomStatusError( "Duplicated Record", 409,
			"Demasiados usuarios duplicados, deberían ser únicos para la busqueda " + arg_id_text
		);
	else if (count == 1)
		return users.next();
	else
...
driver.usr_login = async (correo, pass) => {
	...
	usr = await driver.usr_getByMailL(correo);
...
driver.usr_update = async ({arg_id_text, arg_name, arg_password, arg_mail, arg_type, arg_rfc, arg_private_key_58, arg_clabe}) => {

	const result = await collUsuario.updateOne({id_text: arg_id_text}, { $set: {name: arg_name, password: arg_password, mail: arg_mail, type: arg_type, rfc: arg_rfc, private_key_58:arg_private_key_58, clabe: arg_clabe}});
	console.log(
